...
def create_course_group_tables(connection: Connection) -> None:
    ...
    create_table_if_missing(
        ...
        CREATE TABLE IF NOT EXISTS key_ideas (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            ai_summary_id INTEGER NOT NULL,
            content TEXT NOT NULL,
    ...
    create_table_if_missing(
        ...
        CREATE TABLE saved_summaries (
            ...
            user_id INTEGER NOT NULL,
            ai_summary_id INTEGER NOT NULL,
            saved_at VARCHAR2(32) DEFAULT TO_CHAR(SYSTIMESTAMP, 'YYYY-MM-DD HH24:MI:SS') NOT NULL,
            FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
            FOREIGN KEY (ai_summary_id) REFERENCES ai_summaries(id) ON DELETE CASCADE,
            UNIQUE (user_id, ai_summary_id)
        )
...
def create_indexes(connection: Connection) -> None:
    ...
    create_index_if_missing(
        connection,
        "idx_ai_summaries_group_created",
        "ai_summaries",
        "group_id, created_at",
    )
    create_index_if_missing(
        connection,
        "idx_key_ideas_summary_position",
        "key_ideas",
        "ai_summary_id, position",
    )
    create_index_if_missing(
        connection,
        "idx_saved_summaries_user",
        "saved_summaries",
        "user_id, saved_at",
    )
    create_index_if_missing(
...
def merge_user_rows(
    ...
    connection.execute(
        "UPDATE ai_summaries SET creator_id = ? WHERE creator_id = ?",
        (target_user_id, source_user_id),
    )
    connection.execute(
        """
        INSERT OR IGNORE INTO saved_summaries (user_id, ai_summary_id, saved_at)
        SELECT ?, ai_summary_id, saved_at
        FROM saved_summaries
        WHERE user_id = ?
        """,
        (target_user_id, source_user_id),
    )
    connection.execute("DELETE FROM saved_summaries WHERE user_id = ?", (source_user_id,))
